...
function ScrollToTopButton() {
  ...
  return visible ? (
    <button
      ...
      className="fixed bottom-20 left-4 sm:bottom-24 sm:left-6 md:bottom-8 md:left-8 z-50 group"
            ...
            <Link to="/" className="group flex items-center space-x-3 sm:space-x-4">
              {logo && (
                <div className="relative">
                  ...
                  <img src={logo} alt="AES Garage" className="relative w-14 h-14 sm:w-16 sm:h-16 md:w-20 md:h-20 object-contain" />
                </div>
              )}
              <div className="flex flex-col">
                <span className="text-2xl sm:text-3xl md:text-4xl font-light tracking-[0.2em] text-white">AES</span>
                <span className="text-[10px] sm:text-xs md:text-sm font-light tracking-[0.3em] text-gray-400">GARAGE</span>
              </div>
            </Link>
  ...
  return (
    <Router future={{ v7_startTransition: true, v7_relativeSplatPath: true }}>
      ...
      <div className="min-h-screen bg-black text-white">
        ...
        {/* Telefon Butonu - Sadece Mobilde Görünür ve Sol Altta */}
        <a
          href={`tel:${contactInfo.phone.replace(/\s+/g, '')}`}
          className="fixed bottom-4 left-4 sm:bottom-6 sm:left-6 md:hidden z-50 group"
          aria-label="Telefonla ara"
        >
          <div className="relative">
            <div className="absolute inset-0 rounded-full blur-xl opacity-50 group-hover:opacity-75 transition-opacity" style={{ background: '#DC2626' }}></div>
            <div className="relative w-12 h-12 sm:w-14 sm:h-14 rounded-full flex items-center justify-center shadow-2xl group-hover:scale-110 transition-transform duration-300" style={{ background: 'linear-gradient(135deg, #EF4444, #DC2626)' }}>
        ...
        <a
          ...
          className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 md:bottom-8 md:right-8 z-50 group whatsapp-btn"
        >
          <div className="relative">
