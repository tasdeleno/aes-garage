        ...
        <div className="max-w-7xl mx-auto">
          ...
          <div className={`grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6 md:gap-8 ${!servicesLoaded ? 'hidden' : ''}`}>
            {services.map((service, index) => (
              ...
              >
                <Link to={`/hizmetler/${slugify(service.title)}`} className="block relative h-48 sm:h-64 md:h-80 overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-t from-black via-black/50 to-transparent z-10"></div>
                  {(service.instagramLink || instagramUrl) && (
                    <span
                      onClick={(e) => { e.preventDefault(); e.stopPropagation(); window.open(service.instagramLink || instagramUrl, '_blank'); }}
                      className="absolute top-3 right-3 z-20 flex items-center gap-1.5 px-3 py-1.5 bg-black/60 backdrop-blur-sm border border-white/20 hover:border-pink-500 hover:bg-pink-600/20 transition-all duration-300 cursor-pointer group/ig"
                    >
                ...
                <div className="p-4 sm:p-6 md:p-8">
                  ...
                  <div className="flex flex-col sm:flex-row gap-3 mt-4 sm:mt-6">
                    ...
                    <Link
                      to={`/hizmetler/${slugify(service.title)}`}
                      className="group/btn w-full py-3 sm:py-4 border border-dark-800 hover:border-white hover:bg-white/5 flex items-center justify-center space-x-3 transition-all duration-300 active:scale-[0.98] touch-manipulation"
                    >
                      <span className="text-xs sm:text-sm font-light tracking-widest">DETAY</span>
                    ...
                    <Link
                      to={`/randevu?service=${encodeURIComponent(service.title)}`}
                      className="group/btn w-full py-3 sm:py-4 border border-dark-800 hover:border-red-600 hover:bg-red-600/10 flex items-center justify-center space-x-3 transition-all duration-300 active:scale-[0.98] touch-manipulation"
                    >
